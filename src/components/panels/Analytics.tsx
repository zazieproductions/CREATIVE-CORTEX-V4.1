...
import { buildActivitySeries, buildDomainRadar, buildVelocity } from '../../lib/analytics';
    ...
    <div className="flex items-end gap-[3px] h-full w-full">
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1 group h-full justify-end">
          <div className="w-full rounded-t transition-all" style={{ height: `${(d.value / max) * 100}%`, background: `linear-gradient(180deg, ${DOMAIN_COLORS[DOMAINS[i]] ?? '#22d3ee'}, transparent)`, minHeight: 3 }} />
...
export function Analytics({ notes, visionIndex, ideaVelocity }: AnalyticsProps) {
  ...
  // Deterministic: derives from the vision index rather than wall-clock time so
  // the readout is stable across renders and screenshots.
  const entropy = Math.round(72 + Math.sin(visionIndex / 9) * 6);
