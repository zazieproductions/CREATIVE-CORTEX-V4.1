...
export function CommandPalette({ open, notes, onClose, onOpenNote, onFocusPanel }: CommandPaletteProps) {
  ...
  // `exec` is declared before the key listener below so the effect (and the
  // result rows) share a single stable handler.
  ...
  useEffect(() => {
    if (!open) return;
    setActive(0); // a fresh open starts with the first row selected
