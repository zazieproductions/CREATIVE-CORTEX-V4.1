...
export function Workspace({
  ...
  const zoomAt = (factor: number, cx?: number, cy?: number) => {
    setView((v) => {
      const ns = Math.max(CAMERA.minZoom, Math.min(CAMERA.maxZoom, v.scale * factor));
      const px = cx ?? vp.w / 2;
      const py = cy ?? vp.h / 2;
  ...
  const fit = () => {
    const scale = Math.max(CAMERA.minZoom, Math.min(vp.w / STAGE_W, vp.h / STAGE_H) * 0.96);
  ...
  return (
    ...
    >
      {/* atmospheric backdrop. BASE_URL keeps the public asset resolvable under
          the GitHub Pages sub-path as well as at a domain root. */}
      <img
        src={`${import.meta.env.BASE_URL}atlas-bg.png`}
        alt=""
        className="absolute inset-0 w-full h-full object-cover opacity-[0.08] pointer-events-none mix-blend-screen"
      />
